...
df=get_df(df_path='./sadhabishegam_reviews.csv')
# print(df)
...
with tab1:
   ...
   temp_df_time_filtered_2=filter_time_df(df,previous_n_days_lm=120*2,previous_n_days_ul=120)
   ...
   fig = px.line(temp_df, x="review_date", y="avg_rating",\
                 title='Google Ratings over the past 4 months',hover_data=['num_reviews'])
...
with tab2:
   ...
   fig = px.line(temp_df, x="review_date", y="avg_rating", text="avg_rating",\
                 title='Google Ratings over the past 1 month',hover_data=['num_reviews'])
   fig.update_traces(textposition="bottom right")
   st.plotly_chart(fig, use_container_width=True)
